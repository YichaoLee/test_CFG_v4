...
$(NAME)buildCFG.bc:$(NAME).bc $(BUILDCFG)/buildCFG.so
	opt -load $(BUILDCFG)/buildCFG.so -load /usr/local/lib/libcapd.so -load /usr/local/lib/libibex.so -load /usr/local/lib/libdreal.so -load /usr/lib/libz3.so -load /usr/local/lib/libminisat.so -buildCFG -func=$(FUNC) -name=$(NAME) -bound=$(BOUND) -pre=$(PRE) -line=$(LINE) -mode=$(MODE) -output=$(OUTMODE) -expression=$(CHECK) <$(NAME).bc>   $(NAME)buildCFG.bc
...
clean:
	...
	rm -f $(BRICKPASS)/Release+Asserts/LinearVerify.*
